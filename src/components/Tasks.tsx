...
const Tasks = () => {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 border-t border-zinc-200 dark:border-zinc-800 p-4 overflow-y-auto">
      <h2 className="font-semibold text-xs uppercase text-zinc-400 mb-4 flex items-center gap-2">
        Tasks <Sparkles size={12} className="text-amber-500 animate-pulse"/>
      </h2>
      <input 
        placeholder="Add task..." 
        onKeyDown={(e: any) => { if(e.key === 'Enter' && e.target.value) { addTask(e.target.value); e.target.value = ''; }}} 
        className="w-full p-2 bg-zinc-100 dark:bg-zinc-900 rounded mb-4 text-sm outline-none" 
      />
      {tasks.map((task: Task) => (
