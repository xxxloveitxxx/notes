...
const CommandPalette = () => {
  ...
  return (
    <Command.Dialog
      ...
      className="fixed inset-0 z-[100] flex items-start justify-center pt-[20vh] bg-zinc-950/50 backdrop-blur-sm"
    >
      <div className="w-full max-w-[640px] bg-white dark:bg-zinc-900 rounded-xl shadow-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
        <div className="flex items-center px-4 border-b border-zinc-200 dark:border-zinc-800">
          ...
          <Command.Input
            ...
            className="w-full h-12 bg-transparent outline-none text-sm text-zinc-900 dark:text-zinc-100"
          />
        </div>
        ...
        <Command.List className="max-h-[300px] overflow-y-auto p-2">
          ...
          <Command.Group heading="General" className="px-2 py-1 text-[10px] font-semibold text-zinc-400 uppercase tracking-wider">
            <Command.Item
              onSelect={() => runCommand(() => addNote())}
              className="flex items-center gap-2 px-2 py-2 text-sm rounded-md cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800 aria-selected:bg-zinc-100 dark:aria-selected:bg-zinc-800 transition-colors"
            >
              <Plus size={16} />
              <span>New Note</span>
            </Command.Item>
          ...
          <Command.Group heading="Notes" className="px-2 py-1 text-[10px] font-semibold text-zinc-400 uppercase tracking-wider mt-2">
            {notes.map((note) => (
              <Command.Item
                key={note.id}
                onSelect={() => runCommand(() => setActiveNote(note.id))}
                className="flex items-center gap-2 px-2 py-2 text-sm rounded-md cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800 aria-selected:bg-zinc-100 dark:aria-selected:bg-zinc-800 transition-colors"
              >
                <StickyNote size={16} />
